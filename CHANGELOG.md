...
### Added
- The MCP Settings window can lock `execute_code`'s safety checks. Until now the `safety_checks` argument won unconditionally: the setting supplied only a default, so any client could pass `safety_checks=false` and clear both the source-pattern guard and the compiled-metadata guard — a footgun-guard rather than a boundary, as the tool's own description said. With the lock on, the setting is the only input and the argument is ignored in both directions: a caller can neither turn the guard off nor turn it on. Off by default, so an existing project sees no change. Legitimate reflection, P/Invoke or serialization snippets need the lock cleared, which is deliberately a decision made at the editor rather than one the calling client makes for itself. Both `SAFETY_CHECK_BLOCKED` hints report the locked state, so an agent is told the argument will not help instead of retrying with it.
...
### Changed
- Contributing grants the project owner a licence to relicense contributions, including into proprietary or commercially licensed products. The project stays MIT for everyone; the addition only preserves the option of a paid package built on this code, which the previous MIT-only wording would have foreclosed as soon as a contribution landed from someone else. Contributors keep copyright in their work.
- **Eleven capped tools take a `cursor`** and report a `next_cursor` when a page is cut short, so a long result set can be walked instead of re-read: `find_assets`, `search_files`, `list_directory`, `find_game_objects`, `get_console_logs`, `get_compilation_errors`, `get_component_properties`, `list_shaders`, `get_top_memory_objects`, `frame_debugger_get_events` and `memory_query_references` (`get_hierarchy` already had one). Every one of them used to answer a truncated read by telling the caller to raise the cap — which re-sends the page already read, so walking 500 results 50 at a time cost 2750 records instead of 500. Two of them (`find_assets`, `list_directory`) had no cap parameter at all: they stopped at a hardcoded 50 and 200 and there was no way to see the rest. `find_assets` and `search_files` now sort their paths, because `AssetDatabase.FindAssets` and `Directory.GetFiles` promise no order and paging an unordered result set silently skips and repeats entries between calls. The wording is identical across all of them (`Showing 1-50 of 200; pass cursor=50 for the next page.`) and absent entirely when a response was not truncated, so an unpaged read looks exactly as it did before. A cursor left over from a stale response is answered with `cursor=N is past the end` rather than an empty result — and for `get_console_logs` that is a real answer instead of a fall-through, which would otherwise have handed back the Editor console's own page one.
...
### Removed
- `set_component_property` — deleted; `set_component_properties` does the same write. The singular tool wrapped its one value into a one-key object and handed it to the same `ComponentSerializer.WriteProperties` the plural calls, through the same target/component resolver, so the two differed only in how the caller spells the argument: `property=mass value=5` against `properties={"mass": 5}`. Two schemas, two descriptions and two sets of error codes for one operation, one of them in the default profile where every connection pays for it. What the singular did have was a hard failure: a write that did not land came back as `PROPERTY_SET_FAILED`, while the plural reported `success` with a `failCount` an agent can skip past. That behaviour moved rather than disappeared — `set_component_properties` now returns `PROPERTY_SET_FAILED` when no field applied, so a misspelled property name fails loudly whether one field was sent or ten. The `core` profile lands at 40, and `minimal` carries the plural in the slot the singular held.
...
### Fixed
- `execute_code` no longer imports assets while Play Mode runs. It refreshed the AssetDatabase before every snippet, which mid-play can start an import and a domain reload — wiping the runtime state a Play Mode snippet exists to inspect, and with a hot-reload plugin installed wedging the editor outright. `request_recompile` already refused for this reason; the guard now sits in the shared refresh pipeline, so the escalation ladder and the import are blocked by the same condition. A snippet then runs against whatever is already compiled, which the tool description says.
