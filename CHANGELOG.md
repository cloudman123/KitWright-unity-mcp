...
- `execute_menu_item` refuses a menu path ending in `...` (or the single-character ellipsis, which third-party `[MenuItem]` attributes often use) unless `allow_modal` is passed. That suffix is the editor's own convention for an item that opens a dialog or file picker (`File/Save As...`, `Assets/Import New Asset...`), and those freeze the editor for as long as they are open. It is a convention, not a probe: an item that prompts without the dots still runs. `File/Exit` is refused outright and `allow_modal` does not open it, because it ends the editor rather than blocking it — the reply would never be delivered.
...
### Fixed
- Client configs are written with the project pin in the URL (`http://127.0.0.1:<port>/p/<pin>/`), and both transports now refuse a request whose pin belongs to another project. `HttpMCPTransport` had the check already, but nothing ever wrote a pin into a URL, so it only ever saw pinless paths and waved them through; the broker — which is the default transport — dropped the request path entirely before handing the body to Unity, so a config left on a stale port reached whichever sibling editor now owns that port, and that editor answered and applied the edits to the wrong project. The broker is passed the pin it serves at spawn and replies 404 to a mismatch, mirroring the direct transport. A pinless path is still accepted by both, since configs written before pinning are in the wild; the auto-rewrite upgrades them on the next server start. The broker protocol version is bumped so an already-running broker without the check is replaced rather than reused. The same pinned URL now also appears in the status tooltip, the browser status page and the generated skill's `curl` example, all of which previously named a pinless `8765`.
...
- Scene tools no longer hang the editor on a modal save prompt. `open_scene`, `create_new_scene` and `close_scene` called `EditorSceneManager.SaveCurrentModifiedScenesIfUserWantsTo()`, which opens the native "Scene(s) Have Been Modified" dialog; a modal dialog runs its own message loop, so the editor stops pumping MCP requests and the call sits there until a human clicks a button — with the editor unattended, until the client times out. They now fail with `SCENE_HAS_UNSAVED_CHANGES` naming the dirty scenes, and take `discard_unsaved` to proceed anyway. `save_scene` had the same problem from the other direction: `SaveScene(scene)` on a scene that was never saved opens a file picker, so it now takes an optional `path` and returns `SCENE_HAS_NO_PATH` when neither is available. That `path` is only for a scene that has none: `SaveScene(scene, otherPath)` is Save As, not a copy — it repoints the open scene at a new asset with a new GUID and leaves the original holding the pre-edit content — so passing a different path for an already-saved scene returns `SCENE_ALREADY_HAS_PATH` instead.
