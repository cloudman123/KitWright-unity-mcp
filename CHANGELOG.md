...
- `fetch_docs` returns markdown text rather than a JSON object: a section per page with the URL, the prose, then every code example in its own fenced block. The payload is mostly prose and code, so the JSON form escaped every newline and quote in it, and a client that linkifies the raw result ran the page URL straight into the `","found":true` that followed it — the rendered link was unclickable. URLs are angle-bracketed so the link ends where it should. Repeating the examples after the prose also keeps the runnable code when `max_chars` truncates.
