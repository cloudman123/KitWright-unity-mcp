...
- `fetch_docs` returns the page's code examples as a separate `examples` array alongside the prose, so a caller after a working snippet does not have to find the code inside the text. The examples also survive `max_chars`, which truncates prose first.
