...
- First-connect client approval runs on the broker transport, which is the default — until now the gate only covered the fallback HTTP transport, so the protection most users were told they had never ran. The broker forwards the connecting client's port and the editor resolves the process from it. Existing broker users see the approval dialog once per client executable.
